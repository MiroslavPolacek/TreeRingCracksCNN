...
bash eval_weight_realPost_print_laptopDebug.sh
"""
#######################################################################
#Arguments
#######################################################################
...
###### DETECT IMAGE in normal orientation
    ...
    if True: #SR==0.98:
        # Flatten mask for plotting
        fmask0 = np.zeros(shape=(imgheight, imgheight))
        #print('nmasks', nmasks)
        for m in range(0,r['masks'].shape[2]):
            fmask0 = fmask0 + r['masks'][:,:,m]
        # Ploting lines is moslty for debugging
        file_name = 'image'+ str(image_id)+'_0'
        masked_image = image.astype(np.uint32).copy()
        masked_image = apply_mask(masked_image, fmask0, alpha=0.3)
        plot_lines(image=masked_image,file_name=file_name,
                    path_out=IMAGE_PATH_OUT, SR=SR, gt_masks=gt_mask[:,:,gt_class_id==1], debug=False)
...
###### DETECT IMAGE 90degree
    ...
    if True: #SR==0.98:
        # Flatten mask for plotting
        fmask90 = np.zeros(shape=(imgheight, imgheight))
        #print('nmasks', nmasks)
        for m in range(0,mask_90_back.shape[2]):
            fmask90 = fmask90 + mask_90_back[:,:,m]
        # Ploting lines is moslty for debugging
        file_name = 'image'+ str(image_id)+'_90'
        masked_image = image.astype(np.uint32).copy()
        masked_image = apply_mask(masked_image, fmask90, alpha=0.3)
        plot_lines(image=masked_image,file_name=file_name,
                    path_out=IMAGE_PATH_OUT, SR=SR, gt_masks=gt_mask[:,:,gt_class_id==1], debug=False)
...
###### DETECT IMAGE 45degree
    ...
    if True: #SR==0.98:
        # Flatten mask for plotting
        fmask45 = np.zeros(shape=(imgheight, imgheight))
        #print('nmasks', nmasks)
        for m in range(0,mask_45_back.shape[2]):
            fmask45 = fmask45 + mask_45_back[:,:,m]
        # Ploting lines is moslty for debugging
        file_name = 'image'+ str(image_id)+'_45'
        masked_image = image.astype(np.uint32).copy()
        masked_image = apply_mask(masked_image, fmask45, alpha=0.3)
        plot_lines(image=masked_image,file_name=file_name,
                    path_out=IMAGE_PATH_OUT, SR=SR, gt_masks=gt_mask[:,:,gt_class_id==1], debug=False)
